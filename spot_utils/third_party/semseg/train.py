        ...
        if i % cfg.TRAIN.disp_iter == 0:
            print(
                ...
                "Accuracy: {:4.2f}, Loss: {:.6f}".format(
                    ...
                    cfg.TRAIN.running_lr_decoder,
                    ave_acc.average(),
                    ave_total_loss.average(),
                )
            )

            fractional_epoch = epoch - 1 + 1.0 * i / cfg.TRAIN.epoch_iters
...
def adjust_learning_rate(optimizers, cur_iter, cfg):
    scale_running_lr = (1.0 - float(cur_iter) / cfg.TRAIN.max_iters) ** cfg.TRAIN.lr_pow
...
def main(cfg, gpus):
    ...
    if len(gpus) > 1:
        segmentation_module = UserScatteredDataParallel(
            segmentation_module, device_ids=gpus
        )
        # For sync bn
